...
app/
  ...
  (auth)/
    login/page.tsx                 # Login + registro dual-mode — "Transforming Investments": wrapper .theme-dark (always-dark), MarketDepthField de fondo (FIXED, SIN grid; vignette radial), hero EvolveLogo3D + slogan BLANCO, card glass STICKY que abre como CHOOSER de 2 cuadrados (Iniciar sesión / Crear cuenta) → al picar abre el form (AnimatePresence, botón Volver), narrativa scroll (5 escenas en components/auth/scenes/); auth logic intacta
  (dashboard)/
...
components/
  ...
  brand/
    EvolveGlyph.tsx                # Marca OFICIAL circle-"e" plana en currentColor (anillo abierto ~1h + bucle interior ABIERTO en apertura sup-derecha + barra diagonal saliendo por la boca → lee "e", NO "ø"). FUENTE ÚNICA de la geometría: exporta EVOLVE_OUTER_D/INNER_D/BAR_D que EvolveLogo3D extruye. Pura (sin hooks). Usada en sidebar/header del dashboard
...
- **`EvolveLogo3D`** (`components/brand/EvolveLogo3D.tsx`): tratamiento 3D del logo OFICIAL circle-"e" como hero del login. Geometría COMPARTIDA con `EvolveGlyph` (anillo abierto + bucle interior ABIERTO en la apertura + barra diagonal saliendo por la boca → lee "e", no "ø"), extruida en **CSS 3D** apilando ~16 copias en Z (sin motor 3D / sin dep) → lee como medallón metálico sólido; gira en turntable con `rotateX` inclinado fijo + light-sweep estacionario; caras **BLANCAS** (match wordmark: white→bone), rim grafito, glow teal sutil bajo el medallón. Reduced-motion → vista 3/4 estática; pausa la rotación cuando la pestaña está oculta. `EvolveMark` (nodos) ya NO se usa en el login.
- **`EvolveGlyph`** (`components/brand/EvolveGlyph.tsx`): marca oficial circle-"e" plana (`currentColor`, sin hooks → server-safe). FUENTE ÚNICA de la geometría de marca: exporta `EVOLVE_OUTER_D`/`EVOLVE_INNER_D`/`EVOLVE_BAR_D` (viewBox 120, arcos long-way que dejan la apertura arriba-derecha) que `EvolveLogo3D` extruye — así el glyph 2D y el hero 3D nunca divergen. Reemplaza al `EvolveMark` (nodos) en el sidebar/header del dashboard.
- **Login** (`app/(auth)/login/page.tsx`): reescrito a "Transforming Investments", wrapper `.theme-dark` (always-dark). Backdrop **FIXED** = `MarketDepthField` + blobs de glow + **vignette radial** (SIN ambient-grid — el usuario lo rechazó por "cuadriculado"; las barras de mercado son el único motivo de fondo) + hairline (fijo mientras la narrativa hace scroll). Grid desktop: col1 = hero (`EvolveLogo3D` + wordmark + slogan "Transforming Investments" **en blanco** + statement) en row1 y la **narrativa scroll** en row2 (5 `NarrativeScene`: precios en vivo, **mejores activos**, vs pares, comparar, inteligencia + cierre); col2 = card `glass spotlight-accent` **STICKY** (`lg:sticky lg:top-0 lg:h-dvh`, centrada) siempre accesible. **La card abre como CHOOSER de 2 cuadrados** (tiles `aspect-square` Iniciar sesión / Crear cuenta con icono LogIn/UserPlus); al picar uno, `AnimatePresence mode="wait"` lo intercambia por el form de ese modo (con botón ← Volver al chooser). Orden DOM = hero → card → escenas ⇒ en móvil queda hero compacto → **chooser/form primero** → escenas (auth nunca se entierra). **Lógica de auth 100% preservada** (`signInWithPassword`/`signUp`, redirect, success screen, toggle de modo); `view: 'choose'|'form'` solo envuelve la presentación.
