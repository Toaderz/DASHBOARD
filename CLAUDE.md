...
- **Integridad del pipeline (PR5) — CRÍTICO**: el LLM **no tiene autoridad sobre identificadores**. `buildCandidates` asigna un `candidate_id` opaco y aleatorio por corrida; el schema de `zod` (`LlmArticleSchema`) **no incluye** `source_url`/`source_name`/`date`/`rank`, así que `z.object` descarta esas claves si el modelo (o contenido scrapeado hostil) las emite. El servidor resuelve url/fuente/fecha desde su propio `RawArticle`. Consecuencia: `full_text_md === null` vuelve a ser **señal real** ("Firecrawl falló en esa URL"), no "el LLM se equivocó un carácter". `rank` se asigna del **orden final** (1..n, `selectFinalArticles` devuelve score desc) justo antes del insert.
  - **Aislamiento por artículo**: cada artículo se valida por separado (`safeParse`) y los válidos pasan; `rating`/`signal` fuera del enum descartan **ese** artículo (hay CHECK en DB), `score` se clampea a 0..25, `score_breakdown` basura → ceros conservando el artículo, `actionability` inválido → `null`, `weekly_summary` inválido → campos vacíos sin tumbar la corrida. El insert de `market_news` intenta el lote y, si falla, reintenta **fila a fila** (`insertNewsRowsIsolated`) → una fila mala ya no destruye el brief completo. Telemetría en `market_briefs.metadata.pipeline` (`articles_received`/`valid`/`discarded`/`discard_reasons`/`inserted`/`urls_extracted`/`budget_ms_remaining`).
