...
- **Ordenar por métrica**: todas las columnas numéricas de `WatchlistTable` tienen `sortingFn` personalizado — extraen el valor numérico renderizado (respetando USD/Ann.) y comparan con `numSort` que envía nulls siempre al fondo. Las columnas `helper.display()` necesitan `sortingFn` explícito; sin él TanStack Table no puede ordenar. CCY y actions tienen `enableSorting: false`.
- **Compartir watchlists**: `WatchlistManager` muestra botón Share2 (hover) en watchlists propias. El dialog llama a `GET /api/users/find?email=` (service role) para resolver el email a un `user_id`, luego inserta en `watchlist_shares`. Las watchlists compartidas aparecen en el sidebar del destinatario con icono `Users` (solo lectura) y subtexto `de @username` (email del dueño, cargado en batch por `useWatchlists` al arrancar). El destinatario puede dejar de seguir la lista con el botón X (hover) → dialog de confirmación → DELETE en `watchlist_shares` donde `shared_with_user_id = currentUserId`. PostgREST devuelve el join de `profiles` como array — usar `share.profiles?.[0]?.email`. `currentUserId` se pasa desde `DashboardShell` para distinguir owned vs. shared client-side.
