...
app/
  ...
  (auth)/
    login/page.tsx                 # Login + registro dual-mode — "Transforming Investments": wrapper .theme-dark (always-dark), MarketDepthField de fondo (FIXED, SIN grid; vignette radial), TOP BAR minimalista sticky (EvolveGlyph + wordmark | botones Iniciar sesión/Crear cuenta — estilo Lovable), hero EvolveLogo3D CENTRADO + slogan BLANCO + CTAs, narrativa scroll (5 escenas en components/auth/scenes/). El form ya NO es card sticky/chooser de cuadrados: los botones abren un Dialog (Radix, mismo patrón que el dashboard) con className theme-dark (re-scope dark porque portalea fuera del wrapper) que muestra el form o el success screen; auth logic intacta
  (dashboard)/
  ...
  api/
    market/
      ...
      returns/route.ts             # 🔒 requireUser() (antes de leer el body) — POST batch. DOS MODOS en la misma tabla (ver
                                   #   "Dos consumidores, dos semánticas"): derivado (claves desnudas 1W/1M/6M/YTD/1Y, TTL 6h) y
                                   #   per-periodo (`perf:<PERIODO>` TTL 1h, `cy:<AÑO>` TTL 30d si el año cerró). Fallback a
                                   #   último-bueno en AMBOS; presupuesto de 600 items + deadline 7s + pool de 8
      search/route.ts              # 🔒 requireUser() — Búsqueda de tickers vía **Yahoo Finance** `v1/finance/search` (NO Finnhub, pese
                                   #   al nombre del módulo) + parseSearchQuery (tope de longitud en el primitivo)
    peers/
...
components/
  ...
  brand/
    EvolveGlyph.tsx                # Marca OFICIAL circle-"e" plana en currentColor (anillo exterior CERRADO + bucle interior cerrado arriba, abierto SOLO en boca sup-derecha + barra diagonal con ambos extremos en el rim, saliendo por la boca → lee "e", NO "ø"; fiel a icon-512.png). FUENTE ÚNICA de la geometría: exporta EVOLVE_OUTER_D/INNER_D/BAR_D que EvolveLogo3D extruye. Pura (sin hooks). Usada en sidebar/header del dashboard
    EvolveMark.tsx                 # Marca viva (nodos ascendentes): motivo de EmptyState + EvolveLoader (loop). YA NO se usa en login NI en el sidebar del dashboard (lo reemplazó EvolveGlyph/EvolveLogo3D)
    EvolveLogo3D.tsx               # Hero del login: logo OFICIAL circle-"e" (geometría compartida de EvolveGlyph) extruido en CSS 3D (≈16 capas en Z) girando en turntable inclinado + light-sweep; sin dep. Caras BLANCAS (match wordmark), rim grafito, glow teal sutil. Reduced-motion → frame estático 3/4; pausa rotación en tab oculto
  auth/
  ...
  dashboard/
    DashboardShell.tsx             # Layout principal: sidebar (EvolveGlyph oficial + wordmark Evolve + disparador ⌘K) + nav + PriceMarquee + CommandPalette montado; header móvil con EvolveGlyph + icono de búsqueda (data-tour attrs)
...
hooks/
  ...
  usePerformanceMetrics.ts         # Retornos históricos (1D→MAX) vía POST bulk a /api/market/returns con claves `perf:`/`cy:`
                                   #   (NO las desnudas de Beating Peers). `planReturnRequests` trocea por AMBOS ejes (≤64 items,
                                   #   ≤16 claves, ≤4 POSTs en vuelo). 40 tickers × 4 columnas: 160 requests → un puñado
  useFxData.ts                     # FX spot rates (1-min) + period returns (5-min). `planFxPairs` deduplica por PAR, no por divisa
                                   #   (GBP/GBX/GBp → un solo GBPUSD=X): 3 divisas × 3 ventanas pasan de 9 requests a 3 (A-5)
...
lib/
  ai/
    ...
    scoring.ts                     # 🔒 Re-derivación SERVER-AUTHORITATIVE de score/rating/signal desde el
                                   #    `score_breakdown` del modelo, con el rubric del propio prompt
                                   #    (readBreakdown/computeTotal/deriveRating/deriveSignal/deriveScoring)
    ...
    scoring.test.ts                # Rubric + bandas + la TABLA old-vs-new que mide el cambio de producto
    news-pipeline.test.ts          # Invariantes de integridad (PR5) + reordenación end-to-end (PR8b)
    normalize-event-tag.test.ts    # Golden tests de la clave de dedup por evento
  auth/
  ...
  market/
    finnhub.ts                     # ⚠️ Nombre HISTÓRICO: hoy pega a **Yahoo Finance**, no a Finnhub (search v1/finance/search +
                                   #   quote v8 chart + campo country). `FINNHUB_API_KEY` no lo lee ningún código (hallazgo A-1)
...
scripts/
  ...
  verify-seed.sql                  # Verificación del trigger chain de alta de usuarios (PR4): inserta un auth.users FALSO, cuenta
                                   #   watchlists/assets y termina en RAISE EXCEPTION → ROLLBACK (nada persiste) + salida visible
  verify-rls.sql                   # 18 casos NEGATIVOS de RLS con dos usuarios reales (PR4): A no lee/modifica a B, no crea shares
                                   #   de B, no ejecuta las seed RPC, no escribe campos curados, no enumera profiles
# ⚠️ `manage-team-evolve.mjs`, `diagnose-seed.sql` y `fix-seed-trigger.sql` estuvieron documentados aquí pero NUNCA se
#    commitearon (hallazgo A-13 de la auditoría). `verify-seed.sql` se reconstruyó desde cero en PR4; la gestión de
#    Team Evolve se hace hoy a mano en el panel de Supabase.
.github/workflows/
...
# FINNHUB_API_KEY              # ⚠️ NO la lee ningún código (A-1): `lib/market/finnhub.ts` usa Yahoo Finance sin API key.
#                                Se deja documentada solo para explicar el nombre del módulo; no hace falta definirla.
...
# ⚠️ `scripts/manage-team-evolve.mjs` está documentado en versiones anteriores pero NO existe en el repo (A-13):
#    el alta de cuentas y el flag is_team_evolve se hacen en el panel de Supabase.
```
...
- **`EvolveLogo3D`** (`components/brand/EvolveLogo3D.tsx`): tratamiento 3D del logo OFICIAL circle-"e" como hero del login. Geometría COMPARTIDA con `EvolveGlyph` (anillo exterior CERRADO + bucle interior cerrado arriba/abierto solo en boca sup-derecha + barra diagonal con ambos extremos en el rim → lee "e", no "ø"; fiel a icon-512.png), extruida en **CSS 3D** apilando ~16 copias en Z (sin motor 3D / sin dep) → lee como medallón metálico sólido; gira en turntable con `rotateX` inclinado fijo + light-sweep estacionario; caras **BLANCAS** (match wordmark: white→bone), rim grafito, glow teal sutil bajo el medallón. Reduced-motion → vista 3/4 estática; pausa la rotación cuando la pestaña está oculta. `EvolveMark` (nodos) ya NO se usa en el login.
- **`EvolveGlyph`** (`components/brand/EvolveGlyph.tsx`): marca oficial circle-"e" plana (`currentColor`, sin hooks → server-safe). FUENTE ÚNICA de la geometría de marca: exporta `EVOLVE_OUTER_D` (círculo cerrado) / `EVOLVE_INNER_D` (bucle abierto solo en boca sup-derecha) / `EVOLVE_BAR_D` (diagonal, extremos en el rim) — viewBox 120 — que `EvolveLogo3D` extruye, así el glyph 2D y el hero 3D nunca divergen. Reemplaza al `EvolveMark` (nodos) en el sidebar/header del dashboard.
- **Login** (`app/(auth)/login/page.tsx`): reescrito a "Transforming Investments", wrapper `.theme-dark` (always-dark). Backdrop **FIXED** = `MarketDepthField` + blobs de glow + **vignette radial** (SIN ambient-grid — el usuario lo rechazó por "cuadriculado"; las barras de mercado son el único motivo de fondo) + hairline. **TOP BAR minimalista sticky** (estilo Lovable): `EvolveGlyph` + wordmark a la izquierda, botones `Iniciar sesión` (ghost) / `Crear cuenta` (primary) a la derecha, siempre alcanzables. Layout de una columna centrada (`max-w-3xl`): hero (`EvolveLogo3D` CENTRADO + wordmark + slogan **en blanco** + statement + CTAs) seguido de la **narrativa scroll** (5 `NarrativeScene`: precios en vivo, **mejores activos**, vs pares, comparar, inteligencia + cierre con CTA). **El form abre en un `Dialog` (Radix, mismo patrón que el dashboard)**: los botones (top bar / hero / cierre) llaman `openAuth(mode)` → `setAuthOpen(true)`. El `DialogContent` lleva `className="theme-dark …"` (re-scope dark porque el Dialog portalea a `<body>`, fuera del wrapper `.theme-dark`) y muestra el form o el success screen ("Revisa tu correo") según `success`. Ya NO hay card sticky ni chooser de cuadrados. **Lógica de auth 100% preservada** (`signInWithPassword`/`signUp`, redirect, success, toggle de modo).
...
- **Compartir watchlists**: `WatchlistManager` muestra Share2 (hover). Dialog resuelve email → `user_id` vía `/api/users/find`, inserta en `watchlist_shares`. El destinatario ve la lista con icono `Users` + subtexto `de @username`. Puede dejar de seguir (DELETE donde `shared_with_user_id = currentUserId`). PostgREST devuelve join de `profiles` como array — usar `share.profiles?.[0]?.email`. El dialog también ofrece **"Team Evolve"** (`addTeamShares` en `useWatchlistShares`): comparte de un golpe con todos los `profiles` con `is_team_evolve=true`. La gestión del equipo (alta de cuentas + flag `is_team_evolve`) se hace fuera de la app, en el panel de Supabase (el `scripts/manage-team-evolve.mjs` que esta guía citaba nunca se commiteó, A-13). ⚠️ El write ya NO lo hace el navegador: `addTeamShares` llama a `POST /api/watchlists/[id]/share-team`, que comprueba la propiedad en servidor, responde **404 y no 403** para no filtrar existencia, es idempotente y devuelve solo `{count}` — el roster del equipo no sale del servidor.
- **Dos consumidores, dos semánticas en `returns_cache` — CRÍTICO (PR6)**: el camino **derivado** (`calculateMultiReturns`, 5 periodos de UNA serie 1Y) y el **per-periodo** (`calculateReturn`, un `range=` propio por periodo) **NO dan el mismo número**. Medido sobre los fixtures golden con los mismos precios: 1W +0,077 pp, 6M −0,095 pp, YTD −0,120 pp. La causa son anclas de fecha distintas (`range=ytd` de Yahoo abre en el cierre del año ANTERIOR; el derivado ancla en la primera barra DEL año). 1M coincide **solo** porque en ese calendario `endMs−30d` cae en la barra que abre `range=1mo` — propiedad del fixture, **no** una regla.
  - Por eso los dos consumidores viven en espacios de nombres separados dentro del mismo JSONB (sin DDL): **Beating Peers** lee las claves **desnudas** (`1W`…`1Y`, derivadas); **watchlist / Top-Bottom** leen `perf:<PERIODO>` y `cy:<AÑO>`, calculadas por `calculateReturnDetailed` — **la misma función** en la que delega `calculateReturn`, así que las cifras son idénticas **por construcción**, no por revisión. El modo derivado **proyecta** solo sus 5 claves y el write **mergea** en vez de reemplazar → ningún consumidor pisa al otro. Congelado en `lib/market/returns-parity.test.ts`.
  - ⚠️ **Nunca unifiques los dos caminos "porque son el mismo periodo"**: reescribiría en silencio todas las cifras del watchlist. Los golden lo ponen rojo.
  - **TTL de `perf:` = 1 h, no 6 h**: esas cifras venían de `/api/market/history?mode=return`, cuyo fetch lleva `revalidate: 3600`, así que 6 h las dejaría **más rancias que antes**. Y un periodo largo no merece TTL más largo: el borde derecho de una ventana 10Y es el cierre de hoy. `cy:<año cerrado>` sí va a 30 días (un año terminado no cambia). `fetched_at` **no** se adelanta en modo per-periodo: es el reloj TTL del bundle derivado, y adelantarlo haría parecer fresco un bundle de peers rancio.
  - `isHealthy` sigue anclado en `1Y != null` **a propósito**: exigir non-null en todo lo pedido refetchearía para siempre a un fondo joven cuyo `6M` es null por definición. La frescura por clave la resuelve el modo per-periodo con su propio `ts:<clave>` — una clave presente con valor `null` y timestamp fresco es un "Yahoo no tiene nada" **conocido** y se sirve.
- **Lease de fundamentals en `quote`** (PR6): ordenar por `fundamentals_fetched_at ASC NULLS FIRST` y cortar a 12 hace que dos invocaciones concurrentes elijan **exactamente los mismos 12 tickers** — con polling de 5 s por pestaña la duplicación es **garantizada**, no incidental. `price_cache.fundamentals_refresh_started_at` (migración 002) es el lease: un `UPDATE … WHERE ticker IN (…) AND (started_at IS NULL OR started_at < cutoff) RETURNING ticker` reclama y reporta en un round-trip. Postgres serializa los updates de la misma fila, así que el perdedor reevalúa su WHERE tras el commit del ganador y recibe conjunto vacío → **los sets son disjuntos por construcción**, sin advisory locks ni tabla extra. Umbral **90 s**, elegido por lo que una invocación puede sobrevivir (Vercel corta a 10 s por defecto, Netlify mata una función síncrona a ~10 s sin honrar `maxDuration`): un lease más viejo es de un proceso muerto, no de uno vivo. Se **libera en la misma escritura** que registra el trabajo (`fundamentals_refresh_started_at: null`), más release explícito de lo reclamado-pero-no-registrado (con `.eq(started_at, claimIso)` para no robar el sello de otro) y el cutoff como backstop. **Falla ABIERTO a propósito**: es un control de eficiencia, no de corrección — si el claim da error (p. ej. la migración 002 aún no está aplicada) se procede con el comportamiento pre-lease y se loguea.
- **Estado explícito en vez de `[]`** (REL-08, PR6): `SeriesStatus = 'ok'|'no_data'|'provider_error'|'stale'` en `fetchHistorySeries`/`calculateReturnDetailed`/`fetchCalendarYearReturnDetailed`. Distinguirlos decide la caché: un `no_data` sobre una cifra ya buena **no la sobrescribe** pero **sí** refresca el timestamp (mata el retry storm sin regresar una cifra); un `provider_error` sirve último-bueno y **no escribe nada** (el siguiente request reintenta). Es **interno** (decisión de caché + logs de `lib/utils/obs.ts`), no se expone en la UI. `fetchHistoricalData`/`calculateReturn`/`fetchCalendarYearReturn` conservan firma y valor exactos → no ripplea a ningún caller.
- **Allowlist en `updateWatchlist`** (A-3): `pickWatchlistUpdates` deja pasar solo `name`/`description`/`selected_metrics`. Antes el objeto del caller iba directo al `.update()`, así que un `{ user_id: otro }` habría **regalado** la watchlist. ⚠️ `WatchlistTable.tsx` todavía escribe `selected_metrics` directo sin pasar por el hook: la columna está en la allowlist, pero la regla no tiene un único punto de aplicación.
...
- **Integridad del pipeline (PR5) — CRÍTICO**: el LLM **no tiene autoridad sobre identificadores**. `buildCandidates` asigna un `candidate_id` opaco y aleatorio por corrida; el schema de `zod` (`LlmArticleSchema`) **no incluye** `source_url`/`source_name`/`date`/`rank`, así que `z.object` descarta esas claves si el modelo (o contenido scrapeado hostil) las emite. El servidor resuelve url/fuente/fecha desde su propio `RawArticle`. Consecuencia: `full_text_md === null` vuelve a ser **señal real** ("Firecrawl falló en esa URL"), no "el LLM se equivocó un carácter". `rank` se asigna del **orden final** (1..n, `selectFinalArticles` devuelve score desc) justo antes del insert.
  - **Aislamiento por artículo**: cada artículo se valida por separado (`safeParse`) y los válidos pasan; `title`/`summary`/`insight`/`candidate_id` vacíos descartan **ese** artículo, `score` se clampea a 0..25 (`Infinity`/`NaN` → 0), `actionability` inválido → `null`, `weekly_summary` inválido → campos vacíos sin tumbar la corrida. ⚠️ **Cambiado por PR8b**: `rating`/`signal` fuera del enum ya **no descartan** el artículo (antes sí, para no chocar con el CHECK de la DB) — el servidor los deriva, así que lo que diga el modelo es solo telemetría y descartar un artículo bien analizado por ese campo sería gratuito. El `score_breakdown` **ya no se degrada a ceros en el schema**: lo normaliza `readBreakdown` (scoring.ts), que distingue «reportó 0» de «no reportó». El insert de `market_news` intenta el lote y, si falla, reintenta **fila a fila** (`insertNewsRowsIsolated`) → una fila mala ya no destruye el brief completo. Telemetría en `market_briefs.metadata.pipeline` (`articles_received`/`valid`/`discarded`/`discard_reasons`/`inserted`/`urls_extracted`/`budget_ms_remaining`).
  - **Prompt injection**: los candidatos van en un bloque **JSON-encodeado** entre un delimitador con **nonce por corrida** (antes `--- ARTICLE n ---`, literal adivinable → un artículo que contuviera esa cadena forjaba un candidato extra). Más la regla 6 del `system` (el contenido es dato, nunca instrucciones → baja `structural_vs_noise`). Capa de robustez, no la defensa principal.
  - **Presupuesto en cascada**: `runNewsPipeline(supabaseAdmin, { budgetMs })` con default **280 s** — por debajo del techo **menor** de los dos entornos (la route HTTP da 300 s; Actions 15 min). Se reparte pipeline → Firecrawl → LLM → DB (`LLM_RESERVE_MS` 130 s, `DB_RESERVE_MS` 20 s); al agotarse **no se inicia trabajo nuevo** en vez de que la plataforma mate el proceso y deje la fila colgada en `generating`.
  - **Firecrawl**: concurrencia acotada a 4 (antes N URLs × 2 intentos × M claves en vuelo). ⚠️ El SDK **no soporta `AbortSignal`** — y es trampa silenciosa: pasarlo a `scrape()` **compila** (la sobrecarga genérica se salta el exceso de propiedades) y se spreadea al cuerpo de la petición sin cancelar nada. El deadline real es `ScrapeOptions.timeout`, que llega al `timeout` de axios. `autoResume: false` **obligatorio**: es nuevo en 4.40.0, viene activado y reemite la petición hasta 5 veces / 20 min tras un timeout → créditos quemados de forma invisible.
  - **429 ≠ cuota agotada** (`classifyFirecrawlError`): `quota` → desactiva la clave para la corrida; `rate-limit` → backoff y reintenta la **misma** clave; `permanent` (404/410/451/robots) → abandona la URL sin recorrer claves ni fallback markdown; `transient` → siguiente clave. Antes un 429 transitorio mataba la clave (el regex mezclaba `rate.?limit` con `quota`) y una ráfaga dejaba la extracción en cero con las dos claves quemadas. **403 se clasifica `transient`** a propósito: es ambiguo (credencial revocada vs. sitio destino que bloquea al scraper) y tratarlo como cuota reintroduce el bug.
- **Re-derivación del scoring (PR8b) — `lib/ai/scoring.ts`**: el modelo devolvía `score`, `rating` y `signal` como campos independientes y nadie los cruzaba nunca contra los 6 sub-scores que él mismo reportaba → filas incoherentes tipo **«rating A con score 4»**. Y `score` no es cosmético: alimenta la aritmética de orden de `dedupeByEvent`, del núcleo A/B de `selectFinalArticles`, de la garantía de portafolio (`score>=11`) y del `rank` final. Ahora el servidor lo calcula:
  - `computeTotal` suma el rubric del **propio prompt** (5 dimensiones + `time_decay`; `portfolio_relevance` NO suma) y **acota cada dimensión a su rango declarado** (0..5; `time_decay` −2..0) **antes** de sumar: sin ese clamp un `macro: 500` recuperaría el control del total por la puerta de atrás. Redondeo a entero (la columna `market_news.score` es `int`) y clamp final 0..25.
  - `deriveRating`/`deriveSignal` usan las bandas del prompt con `>=` descendente → **exhaustivas** (A=19-25/B=15-18/C=11-14/D<11; STRONG>=19/MODERATE 15-18/WEAK<15). Consecuencia estructural: **«rating A con score 4» deja de ser representable** porque rating y señal salen siempre del score persistido, y lo persistido está siempre en el enum del CHECK.
  - **`score_breakdown` ilegible/ausente → respaldo al `score` del modelo, con rating/señal derivados DE ESE score.** Decisión deliberada: re-derivar daría 0 → D/WEAK, o sea enterrar un artículo posiblemente bueno por un fallo de formato en un campo que hasta ahora era informativo. No reabre el agujero (la incoherencia era el bug, y aquí es imposible), el score sigue acotado a 0..25 y es **anómalo y contado** (`breakdown_degraded`). Un breakdown **legítimamente todo a ceros** SÍ se re-deriva a 0 → D/WEAK: `readBreakdown` distingue «reportó 0» de «no reportó» (exige que las 6 contribuyentes al total lleguen como números; `portfolio` no cuenta porque no suma).
  - **Telemetría** en `market_briefs.metadata.pipeline` y en el log `[news-cron]`: `articles_rescored` (rating/señal corregidos frente a una afirmación **válida** del modelo) y `breakdown_degraded`.
  - ⚠️ **Es un cambio de producto VISIBLE: más C/D, menos A/STRONG, y el brief se REORDENA.** Medido en `lib/ai/scoring.test.ts` sobre 12 salidas realistas del modelo: ratings `{A:9,B:1,C:1,D:1}` → `{A:4,B:3,C:2,D:3}`; señales `{STRONG:9,MODERATE:1,WEAK:2}` → `{STRONG:4,MODERATE:3,WEAK:5}`; núcleo A/B 10/12 → 7/12; garantía de portafolio (score>=11) 10/12 → 9/12; 8 de 12 cambian de rating (7 bajan, **1 sube** — no es de un solo sentido). La reordenación end-to-end (incluida la caída de un artículo fuera del conteo 5–7 y el `rank` 1..n) está congelada en `lib/ai/news-pipeline.test.ts`.
  - **NO se tocó**: el rubric, el prompt de análisis, `selectFinalArticles`/`dedupeByEvent`, ni `actionability` (se conserva tal cual aunque el rating derivado baje de A/B, para no perder información).
- **`toValidDate` — fecha PARCIAL → `null`** (corregido en PR8b): `new Date('2026-05')` es válido y devuelve el **1 de mayo**, o sea completaba el día que la fuente nunca dio (invención de dato, justo lo que prohíbe la regla 1 del prompt). El comentario de la función ya declaraba `null`; faltaba la guarda `/^\d{4}(-\d{1,2})?$/`. Un `'2026'` suelto daba `2026-01-01` por la misma vía.
...
- **Scoring — RE-DERIVADO EN SERVIDOR (`lib/ai/scoring.ts`), el modelo solo aporta sub-scores y prosa**: 5 dimensiones 0–5 (`macro_impact`, `surprise_factor`, `market_relevance`, `forward_implications`, `structural_vs_noise`) + `time_decay` → TOTAL máx 25. `portfolio_relevance` NO suma al total (solo informativo + garantía de inclusión). RATING A(19-25)/B(15-18)/C(11-14)/D(<11); SIGNAL STRONG/MODERATE/WEAK. Few-shot de calibración para reducir varianza. ⚠️ El `score`/`rating`/`signal` que emite el modelo **NO se persisten**: `deriveScoring` calcula el TOTAL del `score_breakdown` y saca rating/señal de las bandas (ver "Re-derivación del scoring" abajo). `selectFinalArticles` arma conteo variable 5–7 por umbral de calidad (piso `min=5`; si el núcleo A/B no llega, rellena con los mejores siguientes), con garantía de inclusión por portafolio (≥C/score≥11) sin exceder 7. `MAX_CANDIDATES=10` da headroom para que tras la dedup por evento queden ≥5 distintos
